...
const moduleFlags = [
    ...
    { id: 'CAN_ACCESS_FORUM_MODERATION', label: 'Forum Moderation', icon: MessageSquare },
    { id: 'CAN_ACCESS_VOLUNTEER_CENTER', label: 'Volunteer Center', icon: Heart },
    { id: 'CAN_ACCESS_EVENTS', label: 'Events & Mixers', icon: Calendar },
    { id: 'CAN_ACCESS_PROGRAMS', label: 'Programs', icon: ClipboardList },
    { id: 'CAN_ACCESS_COMMUNITY_FEED', label: 'Community Feed', icon: Rss },
...
const actionFlags = [
    { id: 'CAN_MANAGE_SUBSCRIPTIONS', label: 'Manage Subscriptions', desc: 'Allow user to edit, cancel, or refund member subscriptions.' },
    { id: 'CAN_APPROVE_DELETIONS', label: 'Approve Account Deletion', desc: 'Required for final approval of GDPR-related deletion requests.' },
    { id: 'CAN_POST_OFFICIAL', label: 'Post to Forum', desc: 'Ability to post official announcements to community boards.' },
];
    ...
    return (
        <div className="max-w-5xl mx-auto pb-32 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="space-y-8">
                ...
                {/* Section 5: Additional Permissions */}
                <section className="bg-surface rounded-2xl border border-border p-8 shadow-sm">
                    <div className="flex items-center gap-3 mb-8">
                        <div className="size-10 bg-tatt-lime/10 rounded-xl flex items-center justify-center text-tatt-lime">
                            <Shield size={24} strokeWidth={2.5} />
                        </div>
                        <h2 className="text-xl font-bold text-foreground">Critical Permissions</h2>
                    </div>
                    <div className="space-y-6">
                        {actionFlags.map((flag) => (
                            <div key={flag.id} className="flex items-center justify-between py-4 border-b border-border last:border-0 last:pb-0">
                                <div className="flex flex-col gap-1">
                                    <p className="font-bold text-foreground">{flag.label}</p>
                                    <p className="text-xs text-tatt-gray">{flag.desc}</p>
                                </div>
                                <label className="inline-flex relative items-center cursor-pointer group">
                                    ...
                                    />
                                    <div className="w-14 h-7 bg-border peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-7 peer-checked:after:border-white after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-[21px] after:w-[21px] after:transition-all peer-checked:bg-tatt-lime"></div>
                                </label>
                            </div>
                        ))}
                    </div>
